...
- **repair**: print — `For stores that must persist, clear the policy: POST /v1/vector_stores/{id} with expires_after set to null. For genuinely temporary stores keep the policy. There is no anchor to confirm: the spec types it as the single literal last_active_at, so a permanent store cannot be expressed as a very long policy, it can only have no policy at all. An anchor with any other value is a platform change to go and read about, not a misconfiguration to fix. Re-ingest any store already in status "expired"; its files are gone and cannot be recovered from the store.`
- **category**: Files and vector stores
- **sources**: https://developers.openai.com/api/docs/api-reference/vector-stores · https://developers.openai.com/api/docs/guides/retrieval
