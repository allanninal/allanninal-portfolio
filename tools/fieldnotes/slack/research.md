...
- **mechanism**: Slack retries an undelivered event up to three times — immediately, after one minute, and after five minutes — carrying `X-Slack-Retry-Num: 1..3` and `X-Slack-Retry-Reason`. A handler that does work *before* acking, and that has no idempotency key, performs that work once per delivery. Each duplicate consumes per-method quota, pushing the app closer to `ratelimited`, which lengthens handler time, which causes more timeouts. It is a positive feedback loop and one of the most-reported production failures in the Bolt repos.
- **detect**: The loop's fingerprint is visible in the workspace itself. `conversations.history?channel=<C...>&limit=200` and look for **duplicate app-authored messages** — same `text`, same `bot_id`, `ts` values within seconds or exactly ~60 and ~300 seconds apart. That 1-minute / 5-minute spacing is the signature of Slack's retry schedule and is unmistakable. Count duplicate clusters per channel and report the rate. Separately, any `ratelimited` observed during the audit compounds the finding.
